...
int main()
{
 ...
 while (i<=j)
 {
    ...
    if(i!=j)
    {
        for(ll p=0;p<vis[i].size();p++)
        {
            cnt[{vis[i][p].first,vis[i][p].second}]=1;
        }
        for(ll p=0;p<vis[j].size();p++)
        {
            cnt[{vis[j][p].first,vis[j][p].second}]=-1;
        }
    }
    else{
         for(ll p=0;p<vis[i].size();p++)
        {
            cnt[{vis[i][p].first,vis[i][p].second}]=0;
        }
