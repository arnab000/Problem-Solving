...
multiset<ll>s;
    ...
    {
        if(!ma3[sura[j].second])
        {
            mx=max(mx,sura[j].first);
            mn=min(mn,sura[j].first);
            s.insert(sura[j].first);
            ma[sura[j].second]=sura[j].first;
              ma3[sura[j].second]=true;
            ma2[j]=true;
            u++;
       ...
       for(ll j=0;j<sura.size();j++)
    {
       ...
       if(!ma2[j])
       {
             s.erase(s.find( ma[sura[j].second]));
             s.insert(sura[j].first);
             ma[sura[j].second]=sura[j].first;
             ma2[j]=true;
