...
void ad(ll p,ll e )
{
   ...
   for(ll i=p;i<=n+2;i+=(i&-i))
   {
     if(e==1)
     fin[i]=add(fin[i],1ll);
     else
     fin[i]=subs(fin[i],1ll);
    ...
    ll m, l;
     ...
     sort(sura.begin(),sura.end());
             for (int i = 0; i < n; i++)
        {
            int curr = sura[i];
            int upper = curr + l;
            int right = upper_bound(sura.begin(),sura.end(), upper) - sura.begin() - 1;
            ans = (ans % mod + nCrModPFermat(right - i, m - 1, mod) % mod) % mod;
        }
