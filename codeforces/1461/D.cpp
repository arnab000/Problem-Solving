...
void solve(ll st,ll en)
{
    if(vis2[{st,en}])
    return;
    if(st<0 || en<0 || st>n-1 || en> n-1)
    return;
    if(st>en)
    return;
    vis2[{st,en}]=true;
    ll sum=cumsum[en]-cumsum[st];
    sum+=sura[st];
   // cout<<sum<<endl;
ma[sum]=true;
    
    ll sea=(sura[st]+sura[en])/2;
    
    ll mid=upper_bound(sura.begin(),sura.end(),sea)-sura.begin();
    solve(st,mid-1);
    solve(mid,en);
...
int main()
{
 ...
 while(test--)
 {
     ...
     cumsum.push_back(sura[0]);
      
     for(ll i=1;i<n;i++)
     {
         cumsum.push_back(cumsum[i-1]+sura[i]);
     }
    solve(0ll,n-1);
     while(m--)
     {
