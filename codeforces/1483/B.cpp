...
int main()
{
 ...
 while(test--)
 {
     ...
     for(ll i=1;i<n;i++)
     {
         ll x=__gcd(sura[i],sura[i-1]);
         if(x==1)
         q.push({i-1,i});
         
     }
     if(__gcd(sura[n-1],sura[0])==1)
     {
         q.push({n-1,0});
     }
     vector<ll>ans;
     while(!q.empty())
     {
        ...
        serial.erase(u.second);
        if(serial.empty())
        break;
        
        auto it =serial.upper_bound(u.second);
      //  cout<<u.second<<" "<<*it<<endl;
        if(it==serial.end())
        {
            it=serial.begin();

        }
        if(__gcd(sura[u.first],sura[*it])==1)
        q.push({u.first,*it});
