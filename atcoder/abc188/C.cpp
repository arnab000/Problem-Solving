...
ll ans=0;
vector<ll>sura;
void solve(vector<ll>winner)
{
    if(winner.size()==2ll)
    {
        if(sura[winner[0]]<sura[winner[1]])
        ans=winner[0];
        else
        ans=winner[1];
        return;
    ...
    vector<ll>nw;
    for(ll i=0;i<winner.size();i+=2)
    {
        if(sura[winner[i]]>sura[winner[i+1]])
        nw.push_back(winner[i]);
        else
        nw.push_back(winner[i+1]);
    }
    solve(nw);
